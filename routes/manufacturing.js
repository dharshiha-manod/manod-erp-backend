 ...
 *   1. Added authenticateToken middleware to every route (was missing)
 *   2. Added  GET  /bom          (was completely missing — caused 404 on load)
 *   3. Added  DELETE /bom/:id    (was completely missing)
 *
...
router.delete("/plans/:id", authenticateToken, (req, res) =>
  send(res, pool.query("DELETE FROM mfg_plans WHERE id=$1", [req.params.id])
    .then(() => ({ success: true }))));

// ══════════════════════════════════════════════════════════════════
// BILL OF MATERIALS
// FIX: GET /bom and DELETE /bom/:id were missing in original file
// ══════════════════════════════════════════════════════════════════

// ── GET all BOMs with their ingredients ──────────────────────────
  ...
  send(res, (async () => {
    ...
    if (ingredients?.length) {
      for (const ing of ingredients) {
        ...
        await pool.query(
          `INSERT INTO mfg_bom_items (bom_id, item_name, quantity, unit, cost) VALUES ($1,$2,$3,$4,$5)`,
          [b.id, ing.item_name, ing.quantity, ing.unit, ing.cost]
        );
      }
    }

    const updatedItems = await pool.query("SELECT * FROM mfg_bom_items WHERE bom_id=$1", [b.id]);
    return { ...b, ingredients: updatedItems.rows };
  })());
});

router.put("/bom/:id", authenticateToken, async (req, res) => {
  const { product_name, product_code, quantity, unit, version, status, notes, ingredients } = req.body;

  send(res, (async () => {
    const bom = await pool.query(
      `UPDATE mfg_bom
       SET product_name=$1, product_code=$2, quantity=$3, unit=$4,
           version=$5, status=$6, notes=$7, updated_at=NOW()
       WHERE id=$8 RETURNING *`,
      [product_name, product_code, quantity, unit, version, status, notes, req.params.id]
    );

    const b = bom.rows[0];
    await pool.query("DELETE FROM mfg_bom_items WHERE bom_id=$1", [b.id]);
...
// FIX: This was completely missing in the original
router.delete("/bom/:id", authenticateToken, async (req, res) => {
  send(res, (async () => {
    await pool.query("DELETE FROM mfg_bom_items WHERE bom_id=$1", [req.params.id]);
    await pool.query("DELETE FROM mfg_bom WHERE id=$1", [req.params.id]);
    return { success: true };
  })());
});
...
router.put("/work-orders/:id", authenticateToken, (req, res) => {
  const { wo_number, product_name, quantity, unit, start_date, end_date, priority, status, assigned_team, progress, notes } = req.body;
  send(res, pool.query(
...
router.delete("/work-orders/:id", authenticateToken, (req, res) =>
...
router.put("/production/:id", authenticateToken, (req, res) => {
  const { ref_no, location, product, quantity, total_cost, date, recipe_used, notes } = req.body;
  send(res, pool.query(
...
router.delete("/production/:id", authenticateToken, (req, res) =>
...
router.put("/resources/:id", authenticateToken, (req, res) => {
  const { name, type, capacity, shift, operator, status, notes } = req.body;
  send(res, pool.query(
...
router.delete("/resources/:id", authenticateToken, (req, res) =>
...
router.post("/machines", authenticateToken, (req, res) => {
...
router.put("/machines/:id", authenticateToken, (req, res) => {
  const { name, machine_code, type, location, manufacturer, model, purchase_date, status, last_maintenance, next_maintenance, notes } = req.body;
  send(res, pool.query(
...
router.delete("/machines/:id", authenticateToken, (req, res) =>
...
router.post("/quality-checks", authenticateToken, async (req, res) => {
...
router.put("/quality-checks/:id", authenticateToken, (req, res) => {
  const { ref_no, product, batch_no, inspected_by, inspection_date, quantity_checked, quantity_passed, quantity_failed, status, remarks } = req.body;
  send(res, pool.query(
...
router.delete("/quality-checks/:id", authenticateToken, (req, res) =>
